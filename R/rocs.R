rocs <- function(Y,pv){
  ...
  plot(c(0,L+0.5,L+0.5,0,0),c(0,0,L+0.5,L+0.5,0),type="l",axes=FALSE,ann=FALSE,,col="blue")
  lines(c(0,0.5),c(L+0.5,L),col='blue')
  text(0.35,L+0.35,bquote(theta))
  text(0.15,L+0.15,bquote(b))
  ...
  for(b in 1:L){
    ...
    text(b,L+0.25,bquote(.(attr(Y,'levels')[b])))
    text(0.25,b-0.5,bquote(.(substr(attr(Y,'levels'),1,7)[L-b+1])))
    Nb = sum(Y==b)
    for(a in 1:L){
